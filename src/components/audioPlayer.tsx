...
import { Previous, Next, Sound } from "iconsax-react";
...
export const Player = () => {
  ...
  return (
    <div className="fixed bottom-0 z-50 -ml-6 w-full border-t border-white/[0.1] bg-[rgba(29,33,35,0.3)] py-4 pl-6 backdrop-blur-lg lg:ml-0 lg:py-6">
      <audio id="audio-player" src={""}></audio>
      <div className="container mx-auto flex items-center justify-between">
        <div className="flex w-3/5 grow-0 items-center md:w-1/5">
          <Image
            src={singleTrack.artworkUrl}
            alt="artwork"
            width={53}
            height={53}
            className="mr-3 rounded-2xl font-bold md:h-12 md:w-12"
          />
          <div className="truncate text-white">
            <p className="text-sm ">{singleTrack.title}</p>
            <span className="text-[10px] text-white/[0.44]">
              {singleTrack.id}
            </span>
          </div>
        </div>
        <div className="mx-9 flex grow-0 flex-col md:w-3/5">
          <div className="flex items-center justify-center md:mb-8">
            <svg
              width="16"
              height="17"
              className="shuffle play-icon mr-11 hidden h-4 md:inline-block"
              viewBox="0 0 16 17"
              fill="none"
            ...
            <Previous className="play-icon mr-11 hidden h-4 md:inline-block" />
            <div className="bg-primary-yellow mr-6 flex h-[25px] w-[25px] items-center justify-center rounded-full text-xs text-white shadow-[0_0_18px_rgba(255,255,255,0.3)] md:mr-11">
              {"" ? (
                <svg
                  width="9"
                  height="10"
            ...
            <Next className="play-icon h-4 hover:cursor-pointer md:mr-11" />
            <svg
              width="16"
              className="repeat fill-primary-yellow play-icon hidden h-4 md:inline-block"
              height="17"
              viewBox="0 0 16 17"
              xmlns="http://www.w3.org/2000/svg"
            >
          ...
          <div className="relative hidden h-1 w-full rounded bg-white/[0.04] md:block">
            <input
              type="range"
              className="absolute hidden h-1 w-full  opacity-0 md:block"
            />
            <div
              className="bg-primary-yellow player h-full rounded"
              style={{ width: Date + "%" }}
            ></div>
            <div
              className="absolute top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full border border-white p-0.5"
              style={{ left: Date + "%" }}
            >
              <div className="bg-primary-yellow h-full w-full rounded-full shadow-[0_0_8px_rgba(0,0,0,0.92)]"></div>
            </div>
          </div>
        </div>
        <div className="hidden w-1/5 grow-0 items-center md:flex">
          <Sound className="mr-2" />
          <div className="volume-wrapper h-1 w-full rounded bg-white/[0.04] hover:cursor-pointer">
            <div
              className="bg-primary-yellow h-full rounded"
